...
import type { HolderType, JeetType } from '../game/types'
...
export function LandingScreen() {
  ...
  return (
    <div style={{
      ...
      background: 'radial-gradient(ellipse at center, #0f1f0f 0%, #0a0e0a 70%)', color: '#fff', fontFamily: 'monospace', overflow: 'auto',
    }}>
      {/* Title */}
      <div style={{ textAlign: 'center', marginBottom: 30 }}>
        <h1 style={{ fontSize: 'clamp(2rem, 6vw, 4rem)', fontWeight: 900, letterSpacing: '-0.02em', marginBottom: 0, color: '#4ade80', textShadow: '0 0 30px rgba(74,222,128,0.5)' }}>
          HOLDERS
        </h1>
        <div style={{ fontSize: 'clamp(1rem, 3vw, 1.5rem)', color: '#fbbf24', fontWeight: 700, margin: '4px 0' }}>VS</div>
        <h1 style={{ fontSize: 'clamp(2rem, 6vw, 4rem)', fontWeight: 900, letterSpacing: '-0.02em', color: '#ef4444', textShadow: '0 0 30px rgba(239,68,68,0.5)', margin: 0 }}>
          JEETS
        </h1>
      </div>
      ...
      <div style={{ maxWidth: 700, width: '90%' }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, justifyContent: 'center' }}>
          {(Object.keys(JEET_DEFS) as JeetType[]).map((type) => {
      ...
      <div style={{ color: '#334155', fontSize: 9, marginTop: 30, textAlign: 'center' }}>
        {TOTAL_WAVES} WAVES · BOSS BATTLES EVERY 10 WAVES
      </div>
    </div>
  )
}
...
function btn(bg: string, hover: string): React.CSSProperties {
  return {
    ...
    padding: '12px 28px', fontSize: 14, fontWeight: 800, fontFamily: 'monospace',
    cursor: 'pointer', letterSpacing: '0.05em', boxShadow: `0 4px 0 ${hover}, 0 6px 20px rgba(0,0,0,0.3)`,
    transition: 'transform 0.05s', textTransform: 'uppercase',
  }
}
