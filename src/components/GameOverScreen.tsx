...
export function GameOverScreen() {
  ...
  return (
    <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: 'radial-gradient(ellipse at center, #1a0a0a 0%, #0a0e0a 70%)', color: '#fff', fontFamily: 'monospace' }}>
      <h1 style={{ fontSize: 'clamp(2rem, 5vw, 3.5rem)', color: '#ef4444', textShadow: '0 0 30px rgba(239,68,68,0.5)', marginBottom: 8 }}>REKT</h1>
      ...
      {!saved ? (
        <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap', justifyContent: 'center' }}>
          ...
          <button onClick={handleSave} disabled={!name.trim()} style={{ background: name.trim() ? '#fbbf24' : '#333', color: '#000', border: 'none', borderRadius: 6, padding: '10px 20px', fontFamily: 'monospace', fontSize: 14, fontWeight: 700, cursor: name.trim() ? 'pointer' : 'not-allowed' }}>SAVE SCORE</button>
        </div>
